...
namespace ICS.Mobile.Pages.Workflow.Components
{
    public partial class Files
    {
        ...
        private int min = 0;
        private int max = 0;
        private List<ImageInsertInput>? Images { set; get; }
        ...
        private async Task DeleteFile(long FileId)
        {
            ...
            {
                if (await Confirm("Are you sure you want to delete this File?", "Delete Screenshot"))
                {
                    var existingFile = Images.FirstOrDefault(i => i.Id == FileId);
        ...
        public async Task FileChanged(InputFileChangeEventArgs e, long fileId)
        {
            ...
            try
            {
                CurrentImage = Images!.FirstOrDefault(i => i.Id == fileId);

                if (CurrentImage is null)
                {
                    ErrorMessage = "No file slot selected";
                    return;
                }
                            ...
                            foreach (string fileName in fileNames)
                            {
                                if (fileName.StartsWith(input.GeneratedFileNameWithoutExtension))
                                {
                                    ...
                                    catch (Exception ex)
                                    {
                                        ...
                                        System.Diagnostics.Debug.WriteLine($"Failed to hydrate file {input.ImageIndex}: {ex.Message}");
                                    }
                                }
                            }
                        }
