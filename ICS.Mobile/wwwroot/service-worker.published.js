...
async function onFetch(event) {
    ...
    // Special handling for module imports from /modules/ directory
    if (requestUrl.pathname.includes('/modules/') && requestUrl.pathname.endsWith('.js')) {
        const cache = await caches.open(cacheName);
        ...
        if (cachedResponse) {
            ...
            try {
                ...
                console.error('Error reading cached module:', e);
            }
        }
        ...
        try {
            ...
            console.error('Module fetch failed for:', request.url, error);
            if (cachedResponse) {
                return cachedResponse;
            }
        }
        ...
        return new Response(
            `// Failed to load module: ${request.url}
export default {};
export function init(settings) { 
    console.warn('Module failed to load offline: ${request.url}'); 
    return true;
}`,
            {
                status: 200,
